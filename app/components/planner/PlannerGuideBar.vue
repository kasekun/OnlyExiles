  ...
  <div class="max-w-[1020px] mx-auto px-6 flex items-center gap-4 py-3 max-sm:px-4 max-sm:py-2">
    ...
    <div class="flex-1 min-w-0">
      <input
        v-if="!readonly"
        ref="nameInputRef"
        class="w-full bg-transparent border border-transparent rounded-[3px] px-2 py-[0.18rem] text-p-lg font-bold text-p-text tracking-[-0.01em] placeholder:text-p-muted outline-none transition-[border-color,background-color] duration-150 hover:border-p-subtle focus:bg-p-inset focus:border-p-amber-dim truncate"
      ...
      <span
        v-else
        class="block text-p-lg font-bold text-p-text tracking-[-0.01em] truncate px-2 py-[0.18rem]"
